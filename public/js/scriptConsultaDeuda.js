function filtrar() {

    token = getMeta('csrf-token'); // recuperamos el token de session

    if ($.fn.DataTable.isDataTable('#tablaConsultaDeuda')) {
        $('#tablaConsultaDeuda').DataTable().clear();
        $('#tablaConsultaDeuda').DataTable().destroy();
    }
    //$("#cantidadEncontrada").empty(); // hacemos un clear del campo texto contador
    var empresa_desde = $("#empresa_desde").val();
    var empresa_hasta = $("#empresa_hasta").val();
    var concepto = $("#concepto").val();
    var periodo_desde = $("#periodo_desde").val();
    var periodo_hasta = $("#periodo_hasta").val();
    var estado = $("#estado").val();
    //console.log(empresaDesde,empresaHasta,periodo);

    $(".loader").css('visibility', 'visible');
    $.ajax({
        type: 'GET',
        url: 'consultadeuda/resultado',
        data: {
            'empresa_desde': empresa_desde,
            'empresa_hasta': empresa_hasta,
            'concepto': concepto,
            'periodo_desde': periodo_desde,
            'periodo_hasta': periodo_hasta,
            'estado': estado
        },
        dataType: 'json', // a JSON object to send back
        success: function (response) { // What to do if we succeed

            //$("#cantidadEncontrada").append(Object.keys(response.resultados).length + " FUNCIONARIOS ENCONTRADOS");
            var data = response.resultados;

            $('#tablaConsultaDeuda').dataTable({


                initComplete: function () {
                    $('.buttons-pdf').html('<button class="btn btn-danger" title="EXPORTAR A PDF"><i class="far fa-file-pdf"></i></button>')
                    $('.buttons-excel').html('<button class="btn btn-success" title="EXPORTAR A EXCEL"><i class="far fa-file-excel"></i></button>')
                    $('.buttons-print').html('<button class="btn btn-info" title="IMPRIMIR EN PANTALLA"><i class="fas fa-print"></i></button>')
                },
                dom: 'Blfrtip', // Ver creo que se escribe asi Blfrtip - Italo

                buttons: [{
                        extend: 'pdfHtml5',
                        orientation: 'landscape',
                        pageSize: 'A4',
                        exportOptions: {
                            columns: [1, 2, 3, 4, 5, 6, 7, 8]
                        },
                        title: 'PARAMETROS SELECCIONADOS: Empresa Desde: ' + response.parametros.nomInicio +
                            ' // Empresa Hasta: ' + response.parametros.nomFin +
                            ' // Concepto: ' + response.parametros.concepto +
                            ' // Estado: ' + response.parametros.estado +
                            ' // Periodo desde: ' + response.parametros.periodo_desde +
                            ' // Periodo hasta: ' + response.parametros.periodo_hasta,

                        customize: function (doc) {
                            doc.pageMargins = [30, 30, 30, 30];
                            doc.defaultStyle.fontSize = 10;
                            doc.styles.tableHeader.fontSize = 12;
                            doc.styles.title.fontSize = 12;
                            // Remove spaces around page title
                            doc.content[0].text = doc.content[0].text.trim();
                            // Create a footer
                            doc['footer'] = (function (page, pages) {
                                return {
                                    columns: [
                                        'Desarrollo de Software',
                                        {
                                            // This is the right column
                                            alignment: 'right',
                                            text: ['Página ', {
                                                text: page.toString()
                                            }, ' de ', {
                                                text: pages.toString()
                                            }]
                                        }
                                    ],
                                    margin: [10, 0]
                                }
                            });
                            doc['header'] = (function (page, pages) {
                                return {
                                    columns: [
                                        'Sistema UOM - v 1.0',

                                        {
                                            // This is the right column
                                            alignment: 'center',
                                            image: 'data:image/jpeg;base64,'
                                        }
                                    ],
                                    margin: [10, 0]
                                }
                            });
                        },
                    },
                    {
                        extend: 'excelHtml5',
                        orientation: 'portrait',
                        pageSize: 'A4',
                        exportOptions: {
                            columns: [1, 2, 3, 4, 5, 6, 7, 8]
                        },
                        title: ' RESULTADO BUSQUEDA '
                    },
                    {
                        extend: 'print',
                        exportOptions: {
                            columns: [1, 2, 3, 4, 5, 6, 7, 8]
                        },
                        autoPrint: false,
                        title: ' RESULTADO BUSQUEDA '
                    }
                ],

                "data": data,
                "paging": true,
                "scrollY": 300,
                "language": {
                    "url": "./datatables/Spanish.json"
                },


                lengthMenu: [
                    [25, 50, 100, -1],
                    [25, 50, 100, "Todos"]
                ], //Italo

                language: {
                    "decimal": "",
                    "emptyTable": "NO HAY INFORMACION PARA MOSTRAR",
                    "info": "MOSTRANDO _START_ A _END_ DE _TOTAL_ REGISTROS",
                    "infoEmpty": "MOSTRANDO 0 A 0 DE 0 REGISTROS",
                    "infoFiltered": "(FILTRADO DE UN TOTAL DE _MAX_ REGISTROS)",
                    "infoPostFix": "",
                    "thousands": ",",
                    "lengthMenu": "MOSTRAR _MENU_ REGISTROS",
                    "loadingRecords": "CARGANDO...",
                    "processing": "PROCESANDO...",
                    "search": "BUSCAR EMPRESA:",
                    "zeroRecords": "SIN RESULTADOS",
                    "paginate": {
                        "first": "PRIMERO",
                        "last": "ULTIMO",
                        "next": "SIGUIENTE",
                        "previous": "ANTERIOR"
                    }
                },

                columns: [{
                        data: "id_deuda"
                    },
                    {
                        "data": "nom_empresa"
                    },
                    {
                        "data": "tipo_cuenta"
                    },
                    {
                        "data": "periodo",
                        render: function (d) {
                            return moment(d).format("MM/YYYY");
                        }
                    },

                    {
                        "data": "ipte_total_formateado",
                        render: function (d) {
                            if (d) {
                                return d;
                            } else {
                                return "0";
                            }
                        }
                    },

                    {
                        "data": "ipte_depositado_formateado",
                        render: function (d) {
                            if (d) {
                                return d;
                            } else {
                                return "0";
                            }
                        }
                    },
                    {
                        "data": "cant_afil_titulares"
                    },
                    {
                        "data": "titularesEmpresa"
                    },
                    {
                        "data": "fecha_vto",
                        render: function (d) {
                            if (d) {
                                return moment(d).format("DD/MM/YYYY");
                            } else {
                                return "SIN DATOS";
                            }
                        }
                    },
                    {
                        "data": "fecha_acreditacion",
                        render: function (d) {
                            if (d) {
                                return moment(d).format("DD/MM/YYYY");
                            } else {
                                return "SIN DATOS";
                            }
                        }
                    }
                ],
                "columnDefs": [{
                        "targets": [0],
                        "visible": false,
                        "searchable": false
                    },
                    {
                        "targets": [1],
                    },
                    {
                        "targets": [2],
                    },
                    {
                        "targets": [3],
                    },
                    {
                        "targets": [4],
                        /*  "createdCell": function (td, cellData) {
                              if (cellData == 0) {
                                  $(td).css('color', 'red')
                              }
                          },*/
                        className: 'columnaConNumero'
                    },
                    {
                        "targets": [5],

                        className: 'columnaConNumero'
                    },
                    {
                        "targets": [6],
                        className: 'columnaConNumero'
                    },
                    {
                        "targets": [7],
                        className: 'columnaConNumero'
                    },
                    {
                        "targets": [8],
                    },
                    {
                        "targets": [9],
                    }
                ]
            });
            $(".loader").css('visibility', 'hidden');
        },
        error: function (jqXHR, textStatus, errorThrown) { // What to do if we fail
            /*  console.log(JSON.stringify(jqXHR)); */
            console.log("AJAX error: " + textStatus + ' : ' + errorThrown);
        }
    });
}

function getDataCell() {
    var table = $('#tablaConsultaDeuda').DataTable();
    $('#tablaConsultaDeuda tbody').on('click', 'tr', function () {
        var indice = table.row(this).index();
        var clave = table.cell({
            row: indice,
            column: 1
        }).data();
    });
}

function getMeta(metaName) { //// metodo que devuelve el valor  
    const metas = document.getElementsByTagName('meta');
    for (let i = 0; i < metas.length; i++) {
        if (metas[i].getAttribute('name') === metaName) {
            return metas[i].getAttribute('content');
        }
    }
    return '';
}
